...
Ten questions. Ask one at a time with selectable options. Every question has a default. Two questions allow more than one answer and say so.

## Opening choice, before the ten
...
| # | Question as asked | Options shown | Default | Filled from a brief |
|---|---|---|---|---|
| 1 | Which product is this ad for? Share the company name, website, and free text for any relevant addition. | Free text, no options, nothing else shown. Behind the scenes: a saved product is reused; otherwise the website is read, the product saved, and the brand card shown to confirm before question 2. | None | Yes, if the brief names it |
| 2 | Which creative concepts? You can choose more than one. | **Recommended mix**: a narrated demo built on a moment, with three openings to test; a proof-led version built on numbers and breadth; a silent text-driven version for muted feeds. Or pick: Narrated demo. Proof-led. Silent scroller. Customer story, needs footage. | Recommended mix | Suggested |
| 3 | What is the ad about? | Free text. Discuss and tighten before moving on. | None | Yes |
| 4 | Who is it for, and what should it achieve? | Free text for audience. Goal: demo requests, trial signups, awareness. | Demo requests | Yes |
...
- Show all answers as one summary with a proposed ad name. This is the only stop before the build. Wait for a go.
- Record the answers and who filled each one (user, brief, or default) in `QUESTIONNAIRE.md` inside the ad folder.
